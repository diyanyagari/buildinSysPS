...
@Component({
  selector: 'app-jadwal-pelayanan',
  templateUrl: './jadwal-pelayanan.component.html',
  styleUrls: ['./jadwal-pelayanan.component.scss', '../../../kiosk-global.scss'],
  providers: [ConfirmationService]
})

export class JadwalPelayananComponent implements OnInit, OnDestroy {
  data: any;
  show = 3;
  showTitle: any;
  more: boolean;
  defaultData: number;
  lengthDataDefault: any;
  ...
  ngOnInit() {
    this.more = false;
    this.defaultData = 3;
    this.showTitle = 'Tampilkan Lebih Banyak';
    this.data = [];
    this.hideMenu();
    this.getJadwalPelayanan();
  }
  ...
  getJadwalPelayanan() {
    this.httpService.get(`${Configuration.get().klinik1Java}/JadwalPelayananController/getJadwalPelayan`).subscribe(res => {
      this.lengthDataDefault = res.jadwalpelayanan.length;
      this.data = res.jadwalpelayanan.slice(0, this.defaultData);
    });
  }

  showMore(data: string){
    if(data === 'Tampilkan Lebih Banyak') {
      this.showTitle = 'Tampilkan Lebih Sedikit';
      this.defaultData = this.lengthDataDefault;
    } else {
      this.showTitle = 'Tampilkan Lebih Banyak';
      this.defaultData = 3;
