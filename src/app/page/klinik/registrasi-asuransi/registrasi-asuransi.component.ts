...
@Component({
  selector: 'app-registrasi-asuransi',
  templateUrl: './registrasi-asuransi.component.html',
  styleUrls: ['./registrasi-asuransi.component.scss'],
  providers: [ConfirmationService]

})
export class RegistrasiAsuransiComponent implements OnInit, AfterViewInit, OnDestroy {
  form: FormGroup;
  formAktif: boolean;
  versi: any;
  listData: any[];
  ...
  getService() {
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/service/list-generic?table=JenisAlamat&select=*&page=1&rows=1000&condition=and&profile=y').subscribe(table => {
      ...
      this.listJenisAlamat.push({
        label: '--Pilih Jenis Alamat--', value: null
      });
  ...
  getJenisKelamin(data) {
    if (data != '') {
      this.httpService.get(Configuration.get().dataMasterNew + '/jeniskelamin/findByKode/1/1').subscribe(table => {
  ...
  changeJenisKelaminFromTitle(event) {
  ...
  changePegawaiProfile(data) {
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/service/list-generic?table=Pegawai&select=*&page=1&rows=1000&criteria=id.kode&values=' + data.value + '&condition=and&profile=y').subscribe(table => {
      ...
      this.getBulanTahunHariFromLong(table.data.data[0].tglLahir, 'data', 0);
