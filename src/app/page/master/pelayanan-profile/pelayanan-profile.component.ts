...
@Component({
	selector: 'app-pelayanan-profile',
	templateUrl: './pelayanan-profile.component.html',
	styleUrls: ['./pelayanan-profile.component.scss'],
	providers: [ConfirmationService]
})
export class PelayananProfileComponent implements OnInit {
	...
	getDataGrid(page: number, rows: number, cari: any) {
		this.httpService.get(Configuration.get().dataMasterNew + '/pelayananprofile/findAll?page=' + page + '&rows=' + rows + '&dir=namaPelayananProfile&sort=desc&namaPelayananProfile' + cari).subscribe(table => {
			this.listData = table.PelayananProfile;
			this.totalRecords = table.totalRow;
		});
	}

	cari() {
		this.httpService.get(Configuration.get().dataMasterNew + '/pelayananprofile/findAll?page=' + Configuration.get().page + '&rows=' + Configuration.get().rows + '&dir=namaPelayananProfile&sort=desc&namaPelayananProfile=' + this.pencarian).subscribe(table => {
			this.listData = table.PelayananProfile;
		});
	}
	...
	clone(cloned: any) {
		let fixHub = {
			"kdPelayananProfileHead": cloned.data.kdPelayananProfileHead,
			"jamBukaDlmHari": cloned.data.jamBukaDlmHari,
			"hariBukaDlmMinggu": cloned.data.hariBukaDlmMinggu,
			"namaPelayananProfile": cloned.data.namaPelayananProfile,
			"qtyBukaDlmMinggu": cloned.data.qtyBukaDlmMinggu,
	...
	simpan() {
		if (this.formAktif == false) {
			...
			this.httpService.post(Configuration.get().dataMasterNew + '/pelayananprofile/save', this.form.value).subscribe(response => {
	...
	update() {
		this.httpService.update(Configuration.get().dataMasterNew + '/pelayananprofile/update', this.form.value).subscribe(response => {
	...
	hapus() {
		this.httpService.delete(Configuration.get().dataMasterNew + '/pelayananprofile/del/' + this.form.get('kode').value).subscribe(response => {
