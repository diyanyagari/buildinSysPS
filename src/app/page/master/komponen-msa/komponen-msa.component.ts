...
// import { Komponen, SelectedItem } from './komponen-msa.interface';
import { Validators, FormControl, FormGroup, FormBuilder } from '@angular/forms';
import { LazyLoadEvent, ConfirmDialogModule, ConfirmationService, MenuItem, DropdownModule } from 'primeng/primeng';
import { AlertService, InfoService, Configuration, FileService, AuthGuard, ReportService } from '../../../global';
@Component({
	selector: 'app-komponen-msa',
	templateUrl: './komponen-msa.component.html',
	styleUrls: ['./komponen-msa.component.scss'],
	providers: [ConfirmationService]
})
export class KomponenMsaComponent implements OnInit {
	selected: any;
	listProduk: any[];
    listJenisKomponen: any[];
	...
	ngOnInit() {
        ...
        this.getKomponen(this.page, this.rows, this.pencarian)
        if(this.listJenisKomponen == undefined){
	...
	downloadPdf() {
		let cetak = Configuration.get().report + '/komponen/laporanKomponen.pdf?kdDepartemen='+this.kddept+'&kdProfile='+this.kdprof+'&gambarLogo=' + this.smbrFile +'&download=true';
		window.open(cetak);
	}

	cetakKomponen(){
		this.laporan = true;
		this.print.showEmbedPDFReport(Configuration.get().report + '/komponen/laporanKomponen.pdf?kdDepartemen='+this.kddept+'&kdProfile='+this.kdprof+'&gambarLogo=' + this.smbrFile +'&download=false', 'frmKomponen_laporanCetak');
    ...
    getKomponen(page: number, rows: number, search: any) {
		this.httpService.get(Configuration.get().dataMasterNew + '/komponen/findAllV2?page='+page+'&rows='+rows+'&dir=noUrut&sort=asc&namaKomponen='+search).subscribe(table => {
    ...
    setNoUrut(totalRecords: number){
        ...
        if (totalRecords <1){
            ...
            this.httpService.get(Configuration.get().dataMasterNew + '/komponen/findAllV2?rows='+totalRecords+'&dir=noUrut&sort=asc').subscribe(table => {
    ...
    loadPageKomponen(event: LazyLoadEvent) {
    	...
    	this.getKomponen((event.rows + event.first) / event.rows, event.rows, this.pencarian);
    	this.page = (event.rows + event.first) / event.rows;
    	this.rows = event.rows;
    }
    getDataKomponen() {
    	this.httpService.get(Configuration.get().dataMasterNew + '/jeniskomponen/findAllData').subscribe(res => {
    		this.listJenisKomponen = [];
    		this.listJenisKomponen.push({ label: '--Pilih Jenis Komponen--', value: '' })
    		for (var i = 0; i < res.JenisKomponen.length; i++) {
    			this.listJenisKomponen.push({ label: res.JenisKomponen[i].namaJenisKomponen, value: res.JenisKomponen[i].kode.kode })
    		};
    	});
    ...
    onRowSelectKomponen(event) {
        ...
        event.data.noUrut =  event.data.noUrut.toString();
    	this.formKomponen.get('noUrut').setValue(event.data.noUrut = event.data.noUrut == '' || event.data.noUrut == null ? '' : parseInt(event.data.noUrut)),
        // this.formKomponen.get('noUrut').enable();
        this.formKomponen.get('kdJenisKomponen').setValue(event.data.kdJenisKomponen),
        this.formKomponen.get('nilaiMin').setValue(event.data.nilaiMin = event.data.nilaiMin == '' || event.data.nilaiMin == null ? '' : parseInt(event.data.nilaiMin)),
        this.formKomponen.get('nilaiNormal').setValue(event.data.nilaiNormal = event.data.nilaiNormal == '' || event.data.nilaiNormal == null ? '' : parseInt(event.data.nilaiNormal)),
        this.formKomponen.get('nilaiMax').setValue(event.data.nilaiMax = event.data.nilaiMax == '' || event.data.nilaiMax == null ? '' : parseInt(event.data.nilaiMax)),
