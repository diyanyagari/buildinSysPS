...
import { Produk, SelectedItem, KelompokProduk, JenisProduk, DetailJenisProduk } from './produk.interface';
...
export class ProdukComponent implements OnInit {
	...
	ngOnInit() {
		...
		if (this.index == 0) {
			this.httpService.get(Configuration.get().dataMasterNew + '/jenisproduk/findAll?page=' + Configuration.get().page + '&rows=' + Configuration.get().rows + '&dir=namaJenisProduk&sort=desc').subscribe(table => {
	...
	onTabChange(event) {
		...
		if (event.index > 0) {
			...
			this.getDetailJenis(this.pageDetailJenisProduk, this.rowsDetailJenisProduk, this.pencarianDetailJenis, data);
		}
		else {
			this.pencarianJenis = '';
			this.formDetailJenisProduk.get('kdJenisProduk').setValue(null);
			this.form.get('kdDetailJenisProduk').setValue(null);
			this.getJenis(this.pageJenisProduk, this.rowsJenisProduk, this.pencarianJenis)
		}
	...
	getJenis(page: number, rows: number, search: any) {
		this.httpService.get(Configuration.get().dataMasterNew + '/jenisproduk/findAll?page=' + page + '&rows=' + rows + '&dir=namaJenisProduk&sort=desc&namaJenisProduk=' + search).subscribe(table => {
	...
	getDetailJenis(page: number, rows: number, search: any, head: any) {
		this.httpService.get(Configuration.get().dataMasterNew + '/detailProduk/findAll?page=' + page + '&rows=' + rows + '&dirnamaDetailJenisProduk&sort=desc&namaDetailJenisProduk=' + search + '&kdJenisProduk=' + head).subscribe(table => {
	...
	getProduk(page: number, rows: number, search: any, head: any) {
		...
		this.httpService.get(Configuration.get().dataMasterNew + '/produk/findAll?page=' + page + '&rows=' + rows + '&dir=namaProduk&sort=desc&namaProduk=' + search + '&kdDetailJenisProduk=' + head).subscribe(table => {
	...
	simpan() {
		if (this.formAktif == false) {
			...
			this.httpService.post(Configuration.get().dataMasterNew + '/produk/save?', this.form.value).subscribe(response => {
