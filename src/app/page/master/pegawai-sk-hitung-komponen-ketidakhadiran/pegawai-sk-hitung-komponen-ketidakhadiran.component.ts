...
@Component({
  selector: 'app-pegawai-sk-hitung-komponen-ketidakhadiran',
  templateUrl: './pegawai-sk-hitung-komponen-ketidakhadiran.component.html',
  styleUrls: ['./pegawai-sk-hitung-komponen-ketidakhadiran.component.scss'],
  providers: [ConfirmationService]
})
export class PegawaiSkHitungKomponenKetidakhadiranComponent implements OnInit {
  ...
  tanggalAkhir: any;
  formAktif: boolean;
  listKetidakhadiran:any[];
  selectedKetidakhadiran:any[];
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      ...
      'kdKetidakhadiran': new FormControl('', Validators.required),
  ...
  getDataGrid(page: number, rows: number, search: any) {
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/pegawaiskpayrollbystatus/findAllKetidakHadiran?page=' + page + '&rows=' + rows +'&dir=status.namaStatus&sort=desc').subscribe(table => {
  ...
  getData(){
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/pegawaiskpayrollbystatus/getRange').subscribe(res => {
      this.listKetidakhadiran = [];
      for (var i = 0; i < res.data.length; i++) {
          this.listKetidakhadiran.push({ label: res.data[i].namaRange, value: res.data[i].id.kode })
      };
      this.selectedKetidakhadiran = this.listKetidakhadiran;
      //this.kdStatusKeterlambatan = res.data[0].kode.kode;
      },
      error => {
          this.listKetidakhadiran = [];
          this.listKetidakhadiran.push({ label: '-- ' + error + ' --', value: '' })
      });
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/service/list-generic?table=KategoryPegawai&select=id.kode,%20namaKategoryPegawai').subscribe(res => {
      ...
      this.httpService.get(Configuration.get().dataMasterNew + '/service/list-generic?table=GolonganPegawai&select=id.kode,%20namaGolonganPegawai').subscribe(res => {
        ...
        this.httpService.get(Configuration.get().dataMasterNew + '/service/list-generic?table=Pangkat&select=id.kode,namaPangkat').subscribe(res => {
          ...
          this.httpService.get(Configuration.get().dataMasterNew + '/pegawaiskpayrollbystatus/getStatusKetidakhadiran').subscribe(res => {
            this.listStatus = [];
            this.listStatus.push({ label: '--Pilih Status Pegawai--', value: '' });
            for (var i = 0; i < res.data.length; i++) {
                this.listStatus.push({ label: res.data[i].namaStatus, value: res.data[i].kode })
            };
  ...
  reset(){
        ...
        this.form.get('kdKetidakhadiran').enable();
...
findSelectedIndex(): number {
  ...
  onRowSelect(event){
    ...
    this.form.get('kdKetidakhadiran').disable();
  ...
  simpan() {
    if (this.formAktif == false) {
        ...
        for (let i = 0; i < this.selectedKetidakhadiran.length; i++) {
            range.push({
                "kode": this.selectedKetidakhadiran[i].value,
            })
        }
...
update() {
  ...
  cari(){
    this.httpService.get(Configuration.get().dataMasterNew + '/pegawaiskpayrollbystatus/findAllKetidakHadiran?page=' + Configuration.get().page + '&rows=' + Configuration.get().rows + '&dir=status.namaStatus&sort=desc&namaStatusPegawai=' + this.pencarian).subscribe(table => {
      this.listData = table.PegawaiSKPayrollByStatus;
      this.totalRecords = table.totalRow;
  });
  }
  ...
  cetak(){
    this.laporan = true;
    this.print.showEmbedPDFReport(Configuration.get().report + '/pegawaiSKPayrollByStatus/laporanPegawaiSKPayrollByStatus.pdf?kdDepartemen='+this.kddept+'&kdProfile='+this.kdprof+'&gambarLogo=' + this.smbrFile +'&download=false', 'frmPegawaiSkHitungKomponenKetidakhadiran_laporanCetak');
  }
