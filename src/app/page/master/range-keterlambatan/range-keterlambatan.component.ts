...
// import { RangeKeterlambatanComponent } from './range-keterlambatan.interface';
import { Validators, FormControl, FormGroup, FormBuilder } from '@angular/forms';
import { LazyLoadEvent, ConfirmDialogModule, ConfirmationService } from 'primeng/primeng';
import { FileService, AlertService, InfoService, Configuration, AuthGuard, ReportService } from '../../../global';
@Component({
  selector: 'app-range-keterlambatan',
  templateUrl: './range-keterlambatan.component.html',
  styleUrls: ['./range-keterlambatan.component.scss'],
  providers: [ConfirmationService]
})
export class RangeKeterlambatanComponent implements OnInit {
  selected: any;
  listDataRange: any[];
  ...
  ngOnInit() {
    ...
    this.httpService.get(Configuration.get().dataMasterNew + '/jenisrange/findKdJenisRangeKeterlambatan').subscribe(res => {
  ...
  convertSatuanSebelumSimpan(dataForm: any){
    ...
    if (dataForm.kdSatuanStandar == this.kdSatuanStandarHari){
      formSubmit.rangeMin = dataForm.rangeMin 
      formSubmit.rangeMax = dataForm.rangeMax 
    }
    else if (dataForm.kdSatuanStandar == this.kdSatuanStandarBulan){
      formSubmit.rangeMin = dataForm.rangeMin * 30
      formSubmit.rangeMax = dataForm.rangeMax * 30
    }
    else if (dataForm.kdSatuanStandar == this.kdSatuanStandarTahun){
      formSubmit.rangeMin = dataForm.rangeMin * 365
      formSubmit.rangeMax = dataForm.rangeMax * 365
    }
    else{
      formSubmit.rangeMin = dataForm.rangeMin
      formSubmit.rangeMax = dataForm.rangeMax
  ...
  convertSatuanPasManggilData(dataRow: any){
    ...
    if (dataRow.kdSatuanStandar == this.kdSatuanStandarHari){
      dataSelect.rangeMin = dataRow.rangeMin 
      dataSelect.rangeMax = dataRow.rangeMax 
    }
    else if (dataRow.kdSatuanStandar == this.kdSatuanStandarBulan){
      dataSelect.rangeMin = dataRow.rangeMin / 30
      dataSelect.rangeMax = dataRow.rangeMax / 30
    }
    else if (dataRow.kdSatuanStandar == this.kdSatuanStandarTahun){
      dataSelect.rangeMin = dataRow.rangeMin / 365
      dataSelect.rangeMax = dataRow.rangeMax / 365
    }
    else{
      dataSelect.rangeMin = dataRow.rangeMin
      dataSelect.rangeMax = dataRow.rangeMax
